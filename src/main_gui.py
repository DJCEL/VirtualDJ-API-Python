...
class VirtualDJMonitor(tk.Tk):
    ...
    def _define_tab_send(self, parent):
        ...
        leftdeck_select_button = ttk.Button(leftdeck_frame, text="SELECT", command=lambda: self._send_command("deck left select"))
        leftdeck_select_button.grid(row=0, column=0, padx=5,pady=5)
        leftdeck_play_button = ttk.Button(leftdeck_frame, text="PLAY", command=lambda: self._send_command("deck left play_button"))
        leftdeck_play_button.grid(row=0, column=1, padx=5,pady=5)

        rightdeck_frame = ttk.LabelFrame(parent, text="Right Deck")
        rightdeck_frame.pack(fill="both", padx=10, pady=5)
        rightdeck_select_button = ttk.Button(rightdeck_frame, text="SELECT", command=lambda: self._send_command("deck right select"))
        rightdeck_select_button.grid(row=0, column=0, padx=5,pady=5)
        rightdeck_play_button = ttk.Button(rightdeck_frame, text="PLAY", command=lambda: self._send_command("deck right play_button"))
        rightdeck_play_button.grid(row=0, column=1, padx=5,pady=5)
